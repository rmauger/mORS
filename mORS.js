...
function javaDOM() {
  ...
  function collapseAllSections(doAddButton) {
    ...
    for (let i = 0; i < collapsibles.length; i++) {
      ...
      sectionDiv.push(buttonElement.parentElement)
      collapseHeight.push(`${buttonElement.scrollHeight}px`)
      ...
      if (doAddButton) {
        buttonElement.addEventListener("click", () => {
          if (sectionDiv[i].style.maxHeight == collapseHeight[i]) {
            expandSingle(buttonElement)
          } else {
            sectionDiv[i].style.maxHeight = collapseHeight[i]
            buttonElement.classList.remove("active")
          }
        })
      }
    }
    for (let i = collapsibles.length; i < collapsibles.length; i--) {
      sectionDiv[i].style.maxHeight = collapseHeight[i]
    }
  }
...
function expandSingle(buttonElement) {
  if (buttonElement) {
    if (buttonElement.classList.contains("collapsible")) {
      ...
      console.log(
        "Target " + buttonElement.innerHTML + " was not an active section"
      )
    }
  } else {
    console.log("")
